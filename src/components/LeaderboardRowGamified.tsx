...
const LeaderboardRowGamified: React.FC<RowProps> = ({ profile, rank, userId }) => {
  ...
  return (
    <div
      className={`
        flex items-center gap-2 sm:gap-3 py-2 sm:py-2 px-2 sm:px-2 my-1 sm:my-1
        ...
        ${isMobile ? "min-h-[60px] shadow-lg border border-white/50" : "min-h-[44px]"}
      `}
    >
      <div className={`text-sm sm:text-xl font-extrabold shrink-0 w-6 sm:w-8 text-center select-none ${isMobile ? "text-base" : ""}`}>
        #{rank + 1}
      </div>
      <div
        className={`rounded-full overflow-hidden border-2 border-white shadow ${isCurrentUser ? "animate-pulse" : ""}`}
        style={{
          width: `clamp(${rowAvatar.mobile}px, 6vw, ${rowAvatar.desktop}px)`,
          height: `clamp(${rowAvatar.mobile}px, 6vw, ${rowAvatar.desktop}px)`,
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <div className={`truncate font-extrabold text-xs sm:text-sm ${isCurrentUser ? "text-green-800" : ""} ${isMobile ? "text-sm" : ""}`}>
        ...
        <span className={`text-[10px] sm:text-xs text-gray-600 mt-0.5 font-semibold flex gap-1 items-center ${isMobile ? "text-xs" : ""}`}>
      ...
      <div className="flex flex-col items-center pr-1 sm:pr-2">
        {rank <= 2 ? (
          <Trophy className={`w-4 h-4 sm:w-6 sm:h-6 ${rank === 0 ? "text-yellow-400" : rank === 1 ? "text-gray-400" : "text-orange-400"} animate-bounce`}
            strokeWidth={2 + rank}
          />
        ) : null}
      </div>
